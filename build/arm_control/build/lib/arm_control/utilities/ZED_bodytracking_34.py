...
# =====================================================
# ZED BODY_34 ARM TRACKING UTILITIES
# =====================================================
#
...
def setup_body_tracking(zed):
    ...
    print("Body tracking enabled with BODY_34 format.")
...
def get_arm_indices(arm="right"):
    ...
    Returns
    -------
    shoulder_idx, elbow_idx, wrist_idx, hand_idx : tuple
...
def get_arm_points(body, arm="right"):
    """
    Extract shoulder, elbow, wrist, and hand points for one arm.
    ...
    for point in [shoulder_2d, elbow_2d, wrist_2d, hand_2d]:
    ...
    for point in [shoulder_3d, elbow_3d, wrist_3d, hand_3d]:
...
def draw_arm_points_and_lines(image, arm_data):
    """
    Draw shoulder, elbow, wrist, and hand on an OpenCV image.
...
def estimate_arm_length(arm_data):
    ...
    # -------------------------------------------------
    # Compute wrist -> hand distance
    # -------------------------------------------------

    wrist_to_hand = np.linalg.norm(hand - wrist)

    # -------------------------------------------------
    # Compute direct shoulder -> hand distance
    # -------------------------------------------------
    #
    # This is not always equal to shoulder_to_elbow
    # plus elbow_to_wrist plus wrist_to_hand unless the arm is straight.
    #
    # -------------------------------------------------
...
def average_arm_data_over_frames(
    ...
    while len(hand_3d_samples) < num_frames:
...
def get_reference_position(
    ...
    # -------------------------------------------------
    # FIXED: Compute center reference point between shoulder and wrist
    # -------------------------------------------------
    center_3d = (shoulder_3d + hand_3d) / 2.0
    ...
    reference_data = {
        ...
        "hand_reference_3d": hand_3d,
        "center_reference_3d": center_3d,  # Added to prevent KeyError downstream
        ...
        "hand_reference_2d": avg_arm_data["hand_2d"],
...
def test_reference_position(
    ...
    while True:
        ...
        if collecting and arm_data is not None:
            ...
            if len(hand_3d_samples) >= num_frames:
                ...
                # Fixed: Also compute center point here for the visualization snapshot
                center_3d = (avg_arm_data["shoulder_3d"] + avg_arm_data["hand_3d"]) / 2.0
                ...
                reference_data = {
                    ...
                    "hand_reference_3d": avg_arm_data["hand_3d"],
                    "center_reference_3d": center_3d,
                ...
                print(
                    "Hand reference:",
                    reference_data["hand_reference_3d"]
                )
                print(
                    "Arm length data:",
        ...
        if not collecting and not collection_done:
            ...
            hand_ref = reference_data["hand_reference_3d"]
            ...
            cv.putText(
                frame,
                f"Hand: x={hand_ref[0]:.3f}, y={hand_ref[1]:.3f}, z={hand_ref[2]:.3f}",
                (20, 105),
                cv.FONT_HERSHEY_SIMPLEX,
                0.6,
                (255, 255, 255),
            ...
            cv.putText(
                frame,
                f"Hand: {lengths['wrist_to_hand']:.3f}",
                (20, 195),
                cv.FONT_HERSHEY_SIMPLEX,
                0.6,
                (255, 255, 255),
