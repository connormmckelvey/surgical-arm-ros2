    ...
    def read_serial_loop(self):
        if self.ser.in_waiting > 0:
            try:
                ...
                self.get_logger().warn(f"Parsing variance encountered: {str(e)}")
    ...
    def handle_parameter_updates(self, params):
        ...
        for param in params:
            if param.name == 'scale':
                ...
                self.get_logger().info(f"Host-side scale factor adjusted to: {self.scale}")
            elif param.name == 'offset':
                self.offset = float(param.value)
                self.get_logger().info(f"Host-side tracking offset shifted to: {self.offset}")
        return result

    def handle_tare_service(self, request, response):
        self.get_logger().info("Host tare command received. Latching next reading to zero status...")
        self.tare_requested = True
        response.success = True
        response.message = "Tare command scheduled inside python math execution pipeline."
        return response
    
    def handle_calibrate_service(self, request, response):
        self.get_logger().info("Calibration command received. Please place a known weight on the sensor for accurate scaling.")
        self.calibration_requested = True
        response.success = True
        response.message = "Calibration command acknowledged. Place known weight and trigger tare to set offset, then adjust scale parameter accordingly."
        return response
