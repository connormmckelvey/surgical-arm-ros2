...
class CameraCalibrationNode(Node):
    def __init__(self):
        ...
        self.final_normal_vector = None  # will be used for orientation reference
        
        # Fixed Visual Artifact Buffers
        self.final_normal_vector_marker_msg = None
        self.centroid_marker_msg = None
        self.mesh_marker_msg = None
        self.planes_marker_msg = None
        self.live_hand_marker_msg = None  # Live updating element
    ...
    def process_and_update_mesh(self):
        ...
        # Build Marker ID 5: Final Normal Vector (Red Arrow)
        self.final_normal_vector_marker_msg = Marker()
        self.final_normal_vector_marker_msg.header.frame_id = "zed_camera_frame"
        self.final_normal_vector_marker_msg.ns = "surface_calibration"
        self.final_normal_vector_marker_msg.id = 5
        self.final_normal_vector_marker_msg.type = Marker.ARROW
        self.final_normal_vector_marker_msg.action = Marker.ADD
        self.final_normal_vector_marker_msg.pose.orientation.w = 1.0
        self.final_normal_vector_marker_msg.scale.x = 0.015  # 1.5 cm shaft thickness
        self.final_normal_vector_marker_msg.scale.y = 0.03   # 3.0 cm arrow head width
        self.final_normal_vector_marker_msg.scale.z = 0.05   # 5.0 cm arrow head length
        self.final_normal_vector_marker_msg.color.r = 1.0
        self.final_normal_vector_marker_msg.color.g = 0.0
        self.final_normal_vector_marker_msg.color.b = 0.0
        self.final_normal_vector_marker_msg.color.a = 1.0

        # Define the Start point (The mesh centroid)
        start_pt = Point(
            x=float(self.final_centroid[0]), 
            y=float(self.final_centroid[1]), 
            z=float(self.final_centroid[2])
        )
        
        # Define the End point (Centroid + Normal Vector)
        # We can scale the vector magnitude (e.g., * 0.25) so it stands out nicely as a 25cm line
        vector_visual_length = 0.25 
        end_pt = Point(
            x=float(self.final_centroid[0] + self.final_normal_vector[0] * vector_visual_length),
            y=float(self.final_centroid[1] + self.final_normal_vector[1] * vector_visual_length),
            z=float(self.final_centroid[2] + self.final_normal_vector[2] * vector_visual_length)
        )

        # Append to the marker's point buffer
        self.final_normal_vector_marker_msg.points.append(start_pt)
        self.final_normal_vector_marker_msg.points.append(end_pt)
    ...
    def send_rviz_delete_markers(self):
        ...
        self.final_normal_vector_marker_msg = self.centroid_marker_msg = self.mesh_marker_msg = self.planes_marker_msg = self.live_hand_marker_msg = self.final_centroid = None
    ...
    def publish_rviz_artifacts(self):
            ...
            if (self.centroid_marker is not None and 
                ...
                self.planes_marker_msg is not None and 
                self.final_normal_vector_marker_msg is not None):
                
                self.centroid_marker.header.stamp = now
                self.mesh_marker_msg.header.stamp = now
                self.planes_marker_msg.header.stamp = now
                self.final_normal_vector_marker_msg.header.stamp = now  
                ...
                self.rviz_pub.publish(self.final_normal_vector_marker_msg)
