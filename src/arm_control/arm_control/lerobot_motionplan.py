...
# Import your custom robotics utilities (ensure these are in your PYTHONPATH)
...
class LerobotMotionPlannerNode(Node):
    def __init__(self):
        ...
        # Subscribes to Node B feedback loop to track where the arm physically is
        ...
        # Subscribes to 3D Cartesian points from external input source
...
#calls command_cartesian_position on the Point recieved
    def cartesian_callback(self, msg):
        """ Triggered whenever a 3D coordinate [x, y, z] is received on the topic. """
        p_des = np.array([msg.x, msg.y, msg.z], dtype=float)
        self.get_logger().info(f"Received new Cartesian target destination: X={msg.x:.3f}, Y={msg.y:.3f}, Z={msg.z:.3f}")
        
        # Process the position target
        self.command_cartesian_position(p_des)

    def compute_forward_kinematics(self):
        """ Computes the 4x4 transformation matrix based on current angles. """
    ...
    def command_cartesian_position(self, p_des):
        """ 
        Calculates IK from a target coordinate [x, y, z] and automatically 
        publishes the 6-DOF joint array command arm/target_joint_angles
        """
        # Seed the numerical solver with the current tracking configuration
        theta_init_rad = np.radians(self.current_joint_angles)
        ...
        # Convert back to degrees for the LeRobot-based driver
    ...
    def command_joint_angles(self, angles_deg):
        """ Direct joint utility override (useful for specific rest/home sequences). """
        msg = Float32MultiArray()
