...
class ZedArmSensorNode(Node):
    def __init__(self):
        ...
        #z is arm up, y is left/right, x is forward/backward in the ZED coordinate system
        #matches lerobot!
        ...
        # ----------------------------------------------------------------------
        # FRAME PROCESSING AND POSE EXTRACTION LOGIC
        # ----------------------------------------------------------------------
    def process_frame_callback(self):
        ...
        if body is not None:
            ...
            if latest_arm_data is not None:
                # Render tracking canvas modifications internally
        ...
        # Crucial: OpenCV requires an explicit frame clock release statement to purge 
